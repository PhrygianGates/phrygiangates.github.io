...
在同一个 \(l\) 分组内的这 \(2l+1\) 个量子态，它们不仅拥有相同的角动量大小，在不受外磁场等因素干扰的情况下，它们还拥有完全相同的能量。这背后的根本原因是描述系统能量的哈密顿算符 \(H\) 在中心力场（如原子核对电子的库仑力）中具有旋转对称性。这种对称性保证了 \(H\) 与总角动量算符 \(L^2\) 及其任意分量（如 \(L_z\)）都是可交换的（commute），即 \([H, L^2] = 0\) 和 \([H, L_z] = 0\)。\([H, L^2] = 0\) 意味着能量本征态可以同时是角动量大小的本征态。对于 \(m_l\) 的简并，并不是单单由 \([H, L_z]=0\) 推出的（它只意味着 \(H\) 和 \(L_z\) 可以有共同本征态），真正导致 \(m_l\) 简并的是 \(H\) 与 \(L_x, L_y\) 也对易（即完整的旋转对称性），这允许我们用升降算符在不同 \(m_l\) 之间“跳跃”而不改变能量。
...
这就引出了著名的泡利不相容原理（Pauli Exclusion Principle）。该原理指出：在一个由费米子（即自旋为半整数的粒子，如电子、质子）组成的系统中，不能有两个或多个粒子处于完全相同的量子态。

对于原子中的电子而言，这意味着没有两个电子可以拥有一组完全相同的量子数 \((n, l, m_l, m_s)\)。

电子是自旋 \(s=1/2\) 的费米子，因此对于由 \((n, l, m_l)\) 确定的每一个空间轨道，自旋投影 \(m_s\) 可以取 \(1/2\) 或 \(-1/2\) 两个值。因此，根据泡利不相容原理，每个空间轨道最多只能容纳两个电子（一个自旋向上，一个自旋向下）。

结合之前关于量子数 \(n\) 和 \(l\) 的讨论，我们可以计算出每个电子壳层能容纳的最大电子数：

*   当 \(n=1\) (K层)：\(l\) 只能为 0。这对应 1 个轨道 (\(m_l=0\))，可容纳 \(1 \times 2 = 2\) 个电子。
*   当 \(n=2\) (L层)：\(l\) 可以为 0 和 1。
    *   \(l=0\) (s轨道) 有 1 个轨道。
    *   \(l=1\) (p轨道) 有 3 个轨道 (\(m_l = -1, 0, 1\))。
    *   总共有 4 个轨道，可容纳 \(4 \times 2 = 8\) 个电子。
*   当 \(n=3\) (M层)：\(l\) 可以为 0, 1, 2。
    *   \(l=0\) (s) 1个轨道，\(l=1\) (p) 3个轨道，\(l=2\) (d) 5个轨道。
    *   总共有 \(1+3+5=9\) 个轨道，可容纳 \(9 \times 2 = 18\) 个电子。

值得注意的是，电子的自旋不仅仅是一个抽象的数学标签，它具有真实的物理效应。由于电子带有电荷且具有自旋，它表现得像一个微小的磁铁（磁偶极子）。如果我们把电子放入一个外磁场 \(\vec{B}\) 中，它的能量就会发生变化，这被称为塞曼效应（Zeeman Effect）。

电子与磁场的相互作用势能为 \(U = -\vec{\mu} \cdot \vec{B}\)，其中 \(\vec{\mu}\) 是电子的磁矩，与自旋 \(\vec{S}\) 成正比。因为自旋在磁场方向上的投影 \(S_z\) 是量子化的（只能取 \(\pm \hbar/2\)），所以电子的能量也只能取两个特定的值：
*   自旋向上：磁矩与磁场同向，能量降低。
*   自旋向下：磁矩与磁场反向，能量升高。